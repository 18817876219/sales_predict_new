...
# 其他可能有用的特征，open_minutes?discounted?
...
def get_exposure_feature(start_date, end_date, train, exposure, TRN_N=7, PRED_N = 7):
    ...
    # TRAIN_TRN_C = [(lambda x: ('expo_before_' + str(TRN_N-x)))(x) for x in range(TRN_N)]
...
def generate_nday_order_info(start_date, end_date, order_cnt, TRN_N = 21, PRED_N = 7):
    ...
    for train_start, train_end, predict_start, predict_end in train_date_zip:
        ...
        for i in sta_date_range:
            ...
            col_name = col_name + ['diff_%s_mean' % i,'mean_%s_decay' % i,'mean_%s' % i,
                                   # 'min_%s' % i,'max_%s' % i,
                                   'std_%s' % i]
...
train = data28[['eleme_restaurant_id','predict_start','day_1']]
train = get_nday_before(data28, 7, train)  # 前n天作为特征
train = get_nday_sta_before(data28, 7, train) # 前n天统计信息作为特征
train = get_nday_sta_before(data28, 14, train) # 前n天统计信息作为特征
train = get_nday_sta_before(data28, 21, train) # 前n天统计信息作为特征
train = get_nday_sta_before(data28, 28, train) # 前n天统计信息作为特征
train = get_store_info(store, train)  # 门店特征
train = get_weather_info(weather,train,store)  # 天气特征
train = remove_data_of_certain_date(train)    # 去掉一些波动数据
train = get_order_feature(train, order_feature) # 订单相关特征
train = get_exposure_feature(start_date, end_date,train, exposure, TRN_N=7)

X = train.drop('day_1',axis=1)
X = X.drop('predict_start',axis=1)
Y = train[(['eleme_restaurant_id','day_1'])]
